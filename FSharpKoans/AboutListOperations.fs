    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec count xs len=
                match xs with
                | [] -> len
                |_::lis ->count lis (len+1) 
            count xs 0  // write a function to find the length of a list
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec one lis =                     
                ...
                | [] -> []
                | a :: b -> 1 + a :: one b
            one xs // write a function which adds 1 to each element
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec doublenum list =                  
                match list with
                | [] -> []
                | head :: tail ->  (head*2) :: doublenum tail
            doublenum xs // write a function which doubles each element
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
             let rec mapping method list =                  
                match list with
                | [] -> []
                | head :: tail ->  f (head) :: mapping method tail
             mapping f xs// write a map which applies f to each element
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec filt method list =                  
                match list with
                | [] -> []
                | head :: tail ->  
                    match f (head) with
                    |true-> head:: filt method tail 
                    |_->filt f tail
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            let rec remove f=
                ...
                |head::tail->
                    match head%2=1 with 
                    |true-> head::(remove tail)
                    |_->remove tail
            remove xs// write a function to filter for odd elements only.
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            let rec fol i list =
                match list with
                | [] -> i
                | head::tail -> fol (i + head) tail
            fol initialState xs // write a function to do what's described above
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec fol i list =
                ...
                | [] -> i
                | head::tail -> fol (i*head) tail
            fol initialState xs // write a function to multiply the elements of a list
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec fol next list = 
                match list with 
                | [] -> next
                | head::tail -> fol (f (next) (head)) tail
            fol initialState xs  // write a function to do a fold.
        fold (+) 0 [1;2;3;4] |> should equal 10
        ...
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
              let rec exi f xs =
                ...
                |head::tail -> 
                          ...
                          |true -> true
                          |false -> exi f tail
              exi f xs // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec part f xs lA lis = 
                match xs with 
                   |[] -> (lA ,lis)
                   |a::tail ->
                             match f a with 
                             |true -> part f tail (lA@[a]) lis
                             |false -> part f tail lA (lis@[a])
            part f xs [] [] // Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec initList lA lis =
                match lA=n with
                | true -> []
                | _ -> f lA  :: initList (lA + 1) lis                     
        ...
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec find list =
                ...
                |head::tail -> 
                       ...
                       |_ -> find tail
                |[] -> None
            find xs  // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
        ...
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            let rec find list =
                ...
                |head::tail -> 
                       ...
                       |_ -> find tail
                |[] -> None
            find xs // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec find p xs lA =
                 match xs with
                 |[] -> List.rev lA
                 |a::tail -> 
                         match p a with 
                         |Some a -> find p tail (a::lA)
                         |None -> find p tail lA
            find p xs [] // Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
             let rec map i list =
                match list with
                | head::tail -> f i head :: map (i + 1) tail
                | [] -> []
             map 0 xs // Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
